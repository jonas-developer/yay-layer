...
function withLiveControls(mapHTML, version) {
  const bar = '<div id="yd-bar" style="position:fixed;right:14px;bottom:14px;z-index:99999;display:flex;gap:8px;align-items:center;font-family:ui-monospace,Menlo,monospace;font-size:12px">'
    ...
    + '<button class="yd-btn yd-primary" id="yd-req" title="Describe a change you want, in plain words. It is queued as a request your AI picks up (it runs `yay requests`) and turns into a polished Brief + specs for you to sign on your phone. You never write the Brief here.">➕ Request a change</button>'
    + '<button class="yd-btn" id="yd-diffs">≷ Changes</button>'
    + '<button class="yd-btn" id="yd-prev" title="Preview: run a package.json script (dev server, build, …) through the dashboard — see its URL + output and stop it.">▷ Preview</button>'
  ...
  const js = '<script>(function(){var V=' + JSON.stringify(version) + ';'
    ...
    + 'document.getElementById("yd-req").onclick=async function(){var m=prompt("Request a change.\\n\\nDescribe what you want built or changed, in plain words — just like you would tell your AI. It becomes a request your AI picks up, turns into a polished Brief + specs, and sends to your phone to sign. You do NOT write the Brief here.\\n\\nWhat do you want?");if(m==null)return;m=String(m).trim();if(!m){show("Request","Type what you want changed.","err");return;}try{var r=await fetch("/api/request/create",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({text:m})});if(r.status===403){show("Request","Add a request from the dashboard on THIS computer (localhost).","err");return;}var j=await r.json();if(j.ok){show("Request","✓ Queued as "+j.id+".\\n\\nYour AI picks this up when it next checks — tell it to \\"check requests\\" now, or it runs `yay requests` at the start of a session. It will draft the Brief + specs and send them to your phone to sign.","ok");}else{show("Request","✗ "+(j.error||"failed"),"err");}}catch(e){show("Request","Could not queue: "+e,"err");}};'
