...
const COMMANDS = [
  ...
  { cmd: 'yay pair [--name you]', desc: 'Pair your phone as the signer — scan the QR; the private key stays on the phone. The FIRST pairing (no roster yet) makes the phone the trust root itself, so no local key is ever needed. Served over HTTPS by default.', flags: [['--name <you>', 'attach the phone key to this identity'], ['--no-https', 'disable TLS (default: mkcert-trusted cert if available, else self-signed)']] },
  ...
  { cmd: 'yay sign [--cell IDs]', desc: 'Approve the current specs — appends a signed seal. Uses THIS project’s signing method automatically (phone or local); no flag needed. If a dashboard is running, the request pops up on the phone you already scanned.', flags: [['--phone / --local', 'force the device (default = the project’s method)'], ['--no-https', 'disable TLS for the phone (on by default)'], ['--mission "<text>"', 'attach a signed Mission — the human-owned headline over this change-set (editable on the phone)'], ['--cell <ids>', 'only these Cells (comma-separated)'], ['--name <signer>', 'which signer']] },
  ...
  { cmd: 'yay dashboard [--port N]', desc: 'Live control panel + phone relay: serves the map (auto-refreshes) with on-demand buttons — Changes, Run tests, Adversary, Regenerate System Plan — AND routes pair/sign/authorize to the phone you scanned ONCE. Leave it running. HTTPS by default; the phone installs the cert from the /trust page for warning-free https.', flags: [['--port <n>', 'port (default 48757)'], ['--open', 'open it in your browser'], ['--no-https', 'disable TLS (default: mkcert-trusted cert if available, else self-signed)']] },
