...
const COMMANDS = [
  ...
  { cmd: 'yay pair [--name you]', desc: 'Pair your phone as the signer — scan the QR; the private key stays on the phone, over your LAN.', flags: [['--name <you>', 'attach the phone key to this identity'], ['--https', 'serve over self-signed TLS (encrypted; one-time cert tap)']] },
  { cmd: 'yay enroll --name X --pubkey <b64>', desc: 'Enroll another signer via an OWNER-signed roster event.', flags: [['--role owner|signer', 'role to grant (default signer)'], ['--by <owner>', 'which owner authorizes it']] },
