...
**5 — The approval ritual.** Present specs as a change-set with a short rationale, then **stop and wait for the human's signature.** Never write trusted code before approval. Report the color you expect each Cell to earn, honestly.
...
*Minimum viable behaviour: decompose the request → write spec blocks → wire `feeds` → present the change-set → wait to be signed → only then write code → predict each Cell's color. Spec-first, every time.*
