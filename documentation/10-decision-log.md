...
**D24. Reverify engine — Hybrid record model + grandfathering posture; verifier capability 1.1.0 → 1.2.0.** Shipped "replay preserved history through today's verifier": `yay reverify --all` reconstructs every Durable snapshot, re-runs the current verifier, and diffs each Cell against its original verdict → a **keyless upgrade report**; `--attest` mints **signed, append-only reverification records** (`kind:"reverification"`, referencing the original — never rewriting old Green); `yay reverify posture off|guarded|strict` is the grandfathering gate control, scopeable per-Cell via a new owner-signed `{ "reverify": "latest" }` policy rule. Adding that policy kind (`reverify-latest`) bumped the derived capability **1.1.0 → 1.2.0** (MINOR — a new policy kind; no existing verdict's meaning changed).
*Why (Hybrid, open report + opt-in signing):* the report is a deterministic recomputation, so it needs no key — anyone can look; only *recording* mints a signed record, so the crown-jewel verifier key stays locked away until a deliberate step. The posture gates on record **existence**, never key-possession. Faithful to the papers (append a new immutable event beside the old; historical truth ≠ current confidence). *Why the posture over an init switch:* enforcement is governance, not architecture — it belongs in owner-signed policy (scopeable, evolvable, attributable), mirroring the foundation seal posture, so it stays out of the capability fingerprint. *Deferred deliberately:* dependency-aware (cross-file) behavioural proving is a separate core-prover project — see [09 — Roadmap](09-roadmap-and-build-plan.md#explicitly-deferred-post-10). Full deliberation (alternatives weighed, open-vs-closed, P5 closed-out) is in the internal decision log `documentation/internal/reverify-record-model.md`.
