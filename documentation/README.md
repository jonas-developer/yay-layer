...
You (or your AI) write a **Cell**: a small block of code with a signed *specification* above it. Before the code is trusted, a **human signs the spec + a plain-English Brief** — never the code bytes. A deterministic, non-AI **verifier** then re-derives, continuously, whether the code still satisfies the signed spec, and colors each Cell Green / Yellow / Red / Unsigned / Pink. A CI **gate** blocks **Red, Unsigned, and Pink** from `main` (Yellow — signed but not machine-proven — is allowed). For agentic workflows, a human can issue a bounded **grant** (Autopilot) letting the AI produce and provisionally act within limits, to be **ratified** later. Every authorization, verification, and ratification is an immutable, content-addressed, attributable record — so years later you can prove *who authorized what, what the machine established, and what changed since*.
